...
def _semantic_local_simplify(circuit):
    ...
    if not diagonal_nodes:
        return _structural_pre_simplify(circuit)
    ...
    if not used_semantic_node:
        rebuilt_circuit = circuit

    return _structural_pre_simplify(rebuilt_circuit)
...
def _lower_semantic_ir_to_target_basis(circuit, semantic_nodes):
    ...
    for node in semantic_nodes:
        if isinstance(node, _SemanticHadamardNode):
            rebuilt_circuit.h(node.qubit_index)
            continue

        if isinstance(node, _SemanticSwapNode):
            _append_swap_in_target_basis(
                rebuilt_circuit, node.left_index, node.right_index
            )
            continue

        if isinstance(node, _SemanticDiagonalPhaseOpNode):
            if node.gate_name in {"rz", "p"}:
                rebuilt_circuit.rz(node.parameter, node.qubit_indices[0])
                continue
...
def _compile_semantic_reference(circuit, compiler):
    """Compile a circuit via direct semantic IR lowering when supported."""
    ...
    return _lower_semantic_ir_to_target_basis(circuit, semantic_ir)
...
def _compile_repeated_composite_prefix_reference(
    ...
    rebuilt_circuit.global_phase = circuit.global_phase
    for _ in range(repeat_count):
        for instruction in optimized_block.data:
            _append_mapped_instruction(
                rebuilt_circuit, optimized_block, instruction
            )
    _append_instruction_sequence(rebuilt_circuit, circuit.data[coverage:])
...
def _supports_semantic_repeated_prefix_fast_path(circuit, repeated_prefix):
    ...
    block_circuit = _semantic_local_simplify(block_circuit)
    return _build_semantic_ir(block_circuit) is not None
    ...
    for node in nodes:
        if isinstance(node, _InstructionSpanNode):
            ...
            if (
                ...
                for instruction in optimized_span.data:
                    _append_mapped_instruction(
                        rebuilt_circuit, optimized_span, instruction
                    )
...
def _compile_backend_repeated_run_reference(circuit, compiler):
    ...
    prefix_compiled = qiskit_transpile(
        prefix_circuit,
        backend=compiler.target_backend,
        optimization_level=0,
        seed_transpiler=getattr(compiler, "seed_transpiler", None),
    )
    suffix_compiled = qiskit_transpile(
        suffix_circuit,
        backend=compiler.target_backend,
        optimization_level=0,
        seed_transpiler=getattr(compiler, "seed_transpiler", None),
    )
    block_compiled = _compile_backend_preset_portfolio(
        block_circuit,
        compiler,
        optimization_level=1,
    )
    ...
    rebuilt_circuit = circuit.copy_empty_like()
    rebuilt_circuit.global_phase = circuit.global_phase
    for instruction in prefix_compiled.data:
        _append_mapped_instruction(
            rebuilt_circuit, prefix_compiled, instruction
        )
    for _ in range(repeat_count):
        for instruction in block_compiled.data:
            _append_mapped_instruction(
                rebuilt_circuit, block_compiled, instruction
            )
    for instruction in suffix_compiled.data:
        _append_mapped_instruction(
            rebuilt_circuit, suffix_compiled, instruction
        )
